...
const updateTeacher = async (name, age, id) => {
  const sql = `UPDATE teacher SET name = ?, age = ? WHERE id = ? RETURNING *`;
  return knex_db.raw(sql, [name, age, id]);
};

const deleteTeacher = async (id) => {
  const sql = `DELETE FROM teacher WHERE id = ? RETURNING *`;
  return knex_db.raw(sql, [id]);
};
...
const addStudent = async (id, name, age, religion) => {
  const sql = `INSERT INTO student (id, name, age, religion) VALUES (?, ?, ?, ?) RETURNING *`;
  return knex_db.raw(sql, [id, name, age, religion]);
};

const updateStudent = async (name, age, religion, id) => {
  const sql = `UPDATE student SET name = ?, age = ?, religion = ? WHERE id = ? RETURNING *`;
  return knex_db.raw(sql, [name, age, religion, id]);
};

const deleteStudent = async (id) => {
  const sql = `DELETE FROM student WHERE id = ? RETURNING *`;
  return knex_db.raw(sql, [id]);
};
